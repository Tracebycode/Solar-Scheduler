...
export default function Dashboard() {
  ...
  if (loading) {
    return <div style={{ padding: "2rem", background: "#1a1a1a", color: "#e0e0e0", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>Loading...</div>;
  }

  if (!state) {
    return <div style={{ padding: "2rem", background: "#1a1a1a", color: "#e0e0e0", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>Failed to load state</div>;
  }
  ...
  return (
    <div style={{ background: "#1a1a1a", color: "#e0e0e0", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      {/* Header Bar */}
      <div style={{ background: "#2d2d2d", padding: "1.5rem 2rem", borderBottom: "2px solid #3d3d3d", boxShadow: "0 2px 4px rgba(0,0,0,0.3)" }}>
        <h1 style={{ margin: 0, fontSize: "1.8rem", fontWeight: "600", color: "#fff" }}>Solar Energy Scheduler</h1>
        <p style={{ margin: "0.5rem 0 0 0", fontSize: "1rem", color: "#b0b0b0" }}>Next 15-Minute Decision Window</p>
      </div>
      ...
      <div style={{ padding: "2.5rem", maxWidth: "1400px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: "1.5rem", marginBottom: "2.5rem", flexWrap: "wrap" }}>
          ...
          <div style={{ 
            ...
            <p style={{ margin: 0, fontSize: "2.5rem", fontWeight: "700", color: "#fff", lineHeight: "1.2" }}>
          ...
          <div style={{ 
            flex: "1", 
            minWidth: "250px", 
            background: state.energyDeficitWh > 0 ? "#4a1a1a" : "#2d2d2d", 
            padding: "2rem", 
            borderRadius: "8px", 
            border: state.energyDeficitWh > 0 ? "1px solid #cc4444" : "1px solid #3d3d3d",
            boxShadow: "0 4px 12px rgba(0,0,0,0.3)"
          }}>
            ...
            <p style={{ margin: 0, fontSize: "2.5rem", fontWeight: "700", color: state.energyDeficitWh > 0 ? "#ff8888" : "#fff", lineHeight: "1.2" }}>
              {state.energyDeficitWh.toFixed(1)} <span style={{ fontSize: "1.4rem", color: "#d0d0d0", fontWeight: "400" }}>Wh</span>
            </p>
            <p style={{ margin: "0.75rem 0 0 0", fontSize: "1rem", color: state.energyDeficitWh > 0 ? "#ffaaaa" : "#d0d0d0" }}>
        ...
        {state.warnings && state.warnings.length > 0 && (
          <div style={{ 
            ...
            <h3 style={{ margin: "0 0 1rem 0", fontSize: "1.2rem", color: "#000", fontWeight: "700" }}>⚠️ Warnings</h3>
            <ul style={{ margin: 0, paddingLeft: "1.5rem" }}>
              {state.warnings.map((warning, index) => (
                <li key={index} style={{ color: "#000", marginBottom: "0.75rem", fontSize: "1.05rem", fontWeight: "500" }}>
                  {warning}
                </li>
              ))}
            </ul>
        ...
        <div style={{ 
          ...
          <h2 style={{ margin: "0 0 1.25rem 0", fontSize: "1.2rem", color: "#fff", fontWeight: "600" }}>Time Window</h2>
          <div style={{ display: "flex", gap: "3rem", flexWrap: "wrap" }}>
            <div>
              <p style={{ margin: "0 0 0.5rem 0", fontSize: "0.9rem", color: "#d0d0d0", fontWeight: "500" }}>Start Time</p>
              <p style={{ margin: 0, fontSize: "1.1rem", color: "#fff", fontWeight: "500" }}>{new Date(state.windowStart).toLocaleString()}</p>
            </div>
            <div>
              <p style={{ margin: "0 0 0.5rem 0", fontSize: "0.9rem", color: "#d0d0d0", fontWeight: "500" }}>End Time</p>
              <p style={{ margin: 0, fontSize: "1.1rem", color: "#fff", fontWeight: "500" }}>{new Date(state.windowEnd).toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: "2rem" }}>
          <h2 style={{ margin: "0 0 1rem 0", fontSize: "1.3rem", color: "#fff", fontWeight: "600" }}>Devices</h2>
          <div style={{ 
            background: "#2d2d2d", 
            borderRadius: "8px", 
            border: "1px solid #3d3d3d",
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {state.devices.map((device, index) => (
                  ...
                  >
                    <td style={{ padding: "1rem", color: "#fff", fontSize: "1rem", fontWeight: "500" }}>{device.name}</td>
                    <td style={{ padding: "1rem", color: "#fff", fontSize: "1rem", fontWeight: "500" }}>{device.type}</td>
                    <td style={{ padding: "1rem", color: "#fff", fontSize: "1rem", fontWeight: "500" }}>{device.powerW}</td>
                    <td style={{ padding: "1rem" }}>
                      <span style={{ 
                        display: "inline-block",
                        padding: "0.35rem 0.75rem",
                        borderRadius: "4px",
                        color: device.isOn ? "#1b5e20" : "#b71c1c",
                        background: device.isOn ? "#4caf50" : "#f44336",
                        fontSize: "0.9rem", 
                        fontWeight: "700",
        ...
        <div style={{ 
          ...
          <div>
            <p style={{ margin: 0, fontSize: "1rem", color: "#d0d0d0" }}>
              <span style={{ color: "#fff", fontWeight: "600" }}>Override Mode:</span> <span style={{ color: "#fff", fontWeight: "500" }}>{state.overrideMode ? "Enabled" : "Disabled"}</span>
            </p>
          </div>
          <div>
            <p style={{ margin: 0, fontSize: "1rem", color: "#d0d0d0" }}>
              Last Updated: <span style={{ color: "#fff", fontWeight: "500" }}>{new Date().toLocaleTimeString()}</span>
            </p>
          </div>
        </div>
      </div>
